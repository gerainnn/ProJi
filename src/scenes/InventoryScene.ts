...
const SLOT_ICON: Record<Slot, string> = { weapon: '⚔', armor: '🛡', trinket: '💎' };
...
export class InventoryScene extends Phaser.Scene {
  ...
  private renderEquipped() {
    ...
    slots.forEach((slot, i) => {
      ...
      const icon = this.add.text(0, -28, SLOT_ICON[slot], { fontSize: '22px', color: THEME.text }).setOrigin(0.5);
      const label = this.add.text(0, -2, SLOT_LABELS[slot], {
        fontFamily: THEME.font.body, fontSize: '12px', color: THEME.textDim, fontStyle: '700',
      }).setOrigin(0.5);
      const name = this.add.text(0, 26, item ? this.shortName(item.name) : 'пусто', {
  ...
  private renderInventory() {
    ...
    items.forEach((item, idx) => {
      ...
      if (isEquipped) {
        const tag = this.add.rectangle(0, -cellH / 2 + 6, 14, 4, color, 1);
        cell.add(tag);
      }
      const icon = this.add.text(-cellW / 2 + 14, -cellH / 2 + 12, SLOT_ICON[item.slot], {
        fontSize: '14px',
      });
      const dmgDef = item.slot === 'weapon' ? `⚔${item.damage}` : item.slot === 'armor' ? `🛡${item.defense}` : `+${item.damage}`;
      const stats = this.add.text(0, -8, dmgDef, {
        fontFamily: THEME.font.body, fontSize: '14px', color: THEME.text, fontStyle: '800',
      }).setOrigin(0.5);
      const name = this.add.text(0, 16, this.shortName(item.name), {
        fontFamily: THEME.font.body, fontSize: '10px', color: THEME.rarity[item.rarity], fontStyle: '700',
      }).setOrigin(0.5);
  ...
  private renderDetail() {
    ...
    const name = this.add.text(-(width / 2 - 20), -detailH / 2 + 14, item.name, {
      fontFamily: THEME.font.body, fontSize: '18px', color, fontStyle: '800',
    }).setOrigin(0, 0);
    const sub = this.add.text(-(width / 2 - 20), -detailH / 2 + 38, `${SLOT_LABELS[item.slot]} • ур. ${item.level} • ${item.rarity}`, {
      fontFamily: THEME.font.body, fontSize: '12px', color: THEME.textDim, fontStyle: '700',
    }).setOrigin(0, 0);
    ...
    if (item.attackSpeed > 0) lines.push(`⏱ Скорость атаки: +${(item.attackSpeed * 100).toFixed(0)}%`);

    const stats = this.add.text(-(width / 2 - 20), -detailH / 2 + 60, lines.join('   '), {
      fontFamily: THEME.font.body, fontSize: '13px', color: THEME.text, fontStyle: '600',
      wordWrap: { width: width - 40 },
    }).setOrigin(0, 0);

    this.detail.add([name, sub, stats]);
