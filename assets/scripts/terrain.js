...
(function (root) {
  ...
  const VIEW_RADIUS     = 8;       // chunks visible in each direction
  const GEN_PER_FRAME   = 2;       // max chunks to generate per frame
  ...
  function ChunkManager(gl, sampler, terrainProgram) {
    ...
    function update(camX, camZ) {
      ...
      // Generate pending chunks (limited per frame for smooth FPS)
      let generated = 0;
      while (pending.length > 0 && generated < GEN_PER_FRAME) {
